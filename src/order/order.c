...
void pullOrders() {
    ...
    if (pf) {
        while (fscanf(pf, "%s%s%lf%s%s%s", orders[totalOrder].id, orders[totalOrder].good_id, &orders[totalOrder].price, \
        orders[totalOrder].date, orders[totalOrder].seller_id, orders[totalOrder].buyer_id) != EOF) totalOrder++;
        fclose(pf);
    }
}
...
void pushOrders() {
    ...
    for (int i = 0; i < totalOrder; i++) {
        printf(pf, "%s %s %.lf %s %s %s\n", orders[i].id, orders[i].good_id, \
        orders[i].price, orders[i].date, orders[i].seller_id, orders[i].buyer_id);
    }
    fclose(pf);
}

int addOrder(Order* o) {
    if(totalOrder == MAX_GOOD) return 0;
    genID(o->id, "O");
    getDate(o->date);
    orders[totalOrder++] = *o;
    return 1;
